...
### def swap(matrix, k):
 ...
 - **В функции используется**:
     ...
     - Если ранг матрицы A < ранга матрицы n, то Слау является вырожденной и используется встроенная команда выхода: [quit()](https://pythonguides.com/python-exit-command/)
 ### def triangle(matrix):
  - Принимает на вход параметр: *matrix* - матрица вида **n+1** x **n**
